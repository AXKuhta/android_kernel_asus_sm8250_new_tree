...
#define GOODIX_I2C_PROBE_WORKER	1
...
// ASUS_BSP +++ Touch
		if (asus_var_panel_stage[0]=='B'){
			...
			r = goodix_i2c_read_trans(dev, dev->reg.coor + pre_buf_len + ((touch_num - 1) * BYTES_PER_COORD),
									  &buffer[pre_buf_len+((touch_num - 1) * BYTES_PER_COORD)], 5);
			if (unlikely(r < 0))
				goto exit_clean_sta;
			chksum = checksum_u8_ys(&buffer[pre_buf_len+((touch_num - 1) * BYTES_PER_COORD)],5);
			if (unlikely(chksum != 0)) {
				ts_debug("Timestamp checksum error:%x", chksum);
				r = -EINVAL;
				goto exit_clean_sta;
			}
			// Version >= FW(06.00.00.05) + CFG(7)
			if (GoodixTSEnTimestamp == true) {
				...
				if (GoodixTSEnTimestampDebug == true ) {
					ts_debug("Timestamp : %d",GoodixTSTimeStamp);
				}
			}
			// Version >= FW(06.00.00.23) + CFG(23)
			TouchArea = (unsigned char)(buffer[pre_buf_len+((touch_num - 1) * BYTES_PER_COORD + 2)]);
			ts_debug("TouchArea %d", TouchArea);
		}
// ASUS_BSP --- Touch
	}
exit_clean_sta:
...
#ifdef GOODIX_I2C_PROBE_WORKER
static void goodix_i2c_probe_worker(struct work_struct *work)
{
	struct goodix_ts_device *ts_device = NULL;
	int r = 0;
	ts_info("goodix_i2c_probe_worker in");

	if (work) {
		ts_device = container_of(work,goodix_ts_device_t,hw_init_worker.work);
		if (!ts_device) {
			printk(KERN_ERR "goodix_i2c_probe_worker: can not get goodix_ts_device\n");
			return;
		}
	} else {
		printk(KERN_ERR "Failed: goodix_i2c_probe_worker: has no data.\n");
		return ;
	}
	...
	if (!goodix_pdev) {
		printk(KERN_ERR "Failed: goodix_i2c_probe_worker: failed to allocate goodix_pdev\n");
	...
	ts_info("goodix_i2c_probe_worker out");
	return ;//eturn r;
...
#endif //#ifdef GOODIX_I2C_PROBE_WORKER
...
#ifdef GOODIX_I2C_PROBE_WORKER
	INIT_DELAYED_WORK(&ts_device->hw_init_worker, goodix_i2c_probe_worker);
	schedule_delayed_work(&ts_device->hw_init_worker,0);
