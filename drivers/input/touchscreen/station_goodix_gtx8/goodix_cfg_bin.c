...
// ASUS_BSP +++ Touch
        if (Station_HWID == 3){ //9896
		ts_dev->reg.gesture = TS_9896_GESTURE_ADDR;
	} else { // 9886
		ts_dev->reg.gesture = TS_9886_GESTURE_ADDR;
	}
// ASUS_BSP --- Touch
...
exit:
	...
	kfree(cfg_bin);
	if (r)
		station_goodix_ts_blocking_notify(NOTIFY_CFG_BIN_FAILED, &r);
	else
		station_goodix_ts_blocking_notify(NOTIFY_CFG_BIN_SUCCESS, &r);
...
static int goodix_read_cfg_bin(struct device *dev, struct goodix_cfg_bin *cfg_bin)
{
	...
	int i = 0, r;
        ...
        ts_info("station hw id %d",Station_HWID);
	/*get cfg_bin_name*/
// ASUS_BSP +++ Touch
        if (Station_HWID == 3){
		strlcpy(cfg_bin_name, TS_9896_CFG_BIN, sizeof(cfg_bin_name));
	} else {
		strlcpy(cfg_bin_name, TS_9886_CFG_BIN, sizeof(cfg_bin_name));
	}
// ASUS_BSP --- Touch
