...
*/
	...
	pr_info("[ASUS_LID] %s : SW_LID report value = %d\n",  __func__, value);
}
...
static int __init lid_driver_probe(struct platform_device *pdev)
{
	...
	printk("[ASUS_LID] %s : set ket capabilities\n", __func__);
	...
	printk("[ASUS_LID] %s : input register ok\n", __func__);
	...
	printk("[ASUS_LID] %s : create sysfs ok\n", __func__);
	...
	printk("[ASUS_LID] %s : request gpio-irq ok\n", __func__);
	...
	printk("[ASUS_LID] %s : request irq handler ok\n", __func__);
	...
	value = !!gpio_get_value_cansleep(ddata->gpio) ^ ddata->active_low;
	printk("[ASUS_LID] Check status, SW_LID report value = %d\n", value);
