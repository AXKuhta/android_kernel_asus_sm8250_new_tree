...
static int ene8k41_regulator_init(struct device *dev)
{
	int ret = 0;

	reg = regulator_get(dev,"ene_vcc");
    if (IS_ERR_OR_NULL(reg)) {
        ret = PTR_ERR(reg);
        printk("[AURA_SYNC] Failed to get regulator ene_vcc %d\n", ret);
        return ret;
    }
    ret = regulator_set_voltage(reg, 2850000, 3000000);
    if (ret) {
        printk("[AURA_SYNC] Failed to set voltage for ene_vcc reg %d\n", ret);
        return -1;
    }

    printk("[AURA_SYNC] ene_vcc regulator setting init");
    return ret;
}
...
// Parse platform data from dtsi
	...
	err = ene8k41_regulator_init(&client->dev);
	if (err) {
		printk("[AURA_SYNC] ene_8k41_parse_dt get fail !!!\n");
		return -ENOMEM;
	}
