...
extern void set_pd2_active(int active);
#endif
...
#ifdef CONFIG_ASUS_PD_CHARGER
	int *chg_rdo, index = -1, cur_uv = 0, cur_ua = 0, j;
	if ((charging_policy & DPM_CHARGING_POLICY_MASK) != DPM_CHARGING_POLICY_MAX_POWER) {
		chg_rdo = chg_eval_src_caps_asus(src_cap->nr, src_cap->pdos);
		index = chg_rdo[0];
		cur_uv = chg_rdo[1];
		cur_ua = chg_rdo[2];
		pr_info("[Bottom_PD] %s pdo-%d: uv (%d), ua (%d), selected!\n", __func__, index, cur_uv, cur_ua);
	}
#endif
...
#ifdef CONFIG_ASUS_PD_CHARGER
	if (((charging_policy & DPM_CHARGING_POLICY_MASK) != DPM_CHARGING_POLICY_MAX_POWER) && index != -1) {
		req_info->pos = index + 1;
		req_info->type = PDO_TYPE_VAL(snk_cap->pdos[index]);
		req_info->vmax = cur_uv / 1000;
		req_info->vmin = cur_uv / 1000;
		req_info->max_ma = cur_ua / 1000;
		req_info->oper_ma = cur_ua / 1000;
		req_info->mismatch = false;
		max_uw = req_info->vmax * req_info->oper_ma;
		for (j = 0; j < snk_cap->nr ; j++) {
			if (req_info->vmax == snk_cap->pdos[j])
...
void pd_dpm_dfp_inform_id(struct pd_port *pd_port, bool ack)
{
	...
	pd_port->tcpc_dev->partner_ident.id_header = pd_port->pe_data.partner_vdos[0];
	pd_port->tcpc_dev->partner_ident.cert_stat = pd_port->pe_data.partner_vdos[1];
	pd_port->tcpc_dev->partner_ident.product = pd_port->pe_data.partner_vdos[2];
