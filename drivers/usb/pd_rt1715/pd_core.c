...
extern const u32 default_rt1715_src_caps[];
extern int default_rt1715_src_caps_size;
extern const u32 default_rt1715_snk_caps[];
extern int default_rt1715_snk_caps_size;
#endif
...
#ifdef CONFIG_ASUS_PD_CHARGER
		pd_port->local_src_cap_default.nr = default_rt1715_src_caps_size;
#endif
		pr_info("%s src pdo data =\n", __func__);
		for (i = 0; i < pd_port->local_src_cap_default.nr; i++) {
#ifdef CONFIG_ASUS_PD_CHARGER
			pd_port->local_src_cap_default.pdos[i] = default_rt1715_src_caps[i];
#endif
			pr_info("%s %d: 0x%08x\n", __func__, i,
...
#ifdef CONFIG_ASUS_PD_CHARGER
		pd_port->local_snk_cap.nr = default_rt1715_snk_caps_size;
#endif
		pr_info("%s snk pdo data =\n", __func__);
		for (i = 0; i < pd_port->local_snk_cap.nr; i++) {
#ifdef CONFIG_ASUS_PD_CHARGER
			pd_port->local_snk_cap.pdos[i] = default_rt1715_snk_caps[i];
#endif
			pr_info("%s %d: 0x%08x\n", __func__, i,
